...
class HexRGBConverter {
    private init() {}
    static func toHex(newColor: CGColor, alpha: Bool = true) -> String? {
        guard let components = newColor.components, components.count >= 3 else {
            return nil
        }
    ...
    static func toCGColor(hex: String) -> CGColor? {
